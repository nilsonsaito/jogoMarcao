...
descer : function(deslocamento) {
    if (this.y <= this.contexto.canvas.height - this.altura - deslocamento) {
        this.y += deslocamento;
    }
},
atirar: function() {
...
colidiuCom: function(outro) {
    if (this.gameLoop.rodando) {
        if (outro instanceof Inimigo) {
            var explosao1 = new Explosao(this.contexto, this.imgExplosao,
                            this.x, this.y);
            this.gameLoop.adicionarSprite(explosao1);
            var explosao2 = new Explosao(this.contexto, this.imgExplosao, 
                            outro.x, outro.y);
            ...
            var nave = this;
				...
				nave.vidasExtras--;
				    ...
				    nave.x = contexto.canvas.width / 2 - imgNave.width / 2;
